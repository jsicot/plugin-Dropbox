...
<?php else: ?>
    ...
    <table>
        ...
        <?php
            // We have to look if there are whole folders to import
            // a $fileNames["_DIR"] subarray has been set in plugin.php::dropbox_save_files
            $dirNames = $fileNames['_DIR'];
            unset($fileNames['_DIR']);
            foreach ($dirNames as $oneDir):
        ?>
        <tr><td><input type="checkbox" name="dropbox-files[]" value="<?php echo html_escape($oneDir["name"]); ?>"/></td><td><span class='dropbox-dir'><?php echo html_escape($oneDir['name']). " (".$oneDir['size'].")"; ?></span></td></tr>
        <?php
            endforeach;
        ?>
